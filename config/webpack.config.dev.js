...
module.exports = {
  ...
  module: {
    ...
    loaders: [
      ...
      // new entry in the `exclude` list for "url" loader. "url" loader embeds assets
      // smaller than specified size as data URLs to avoid requests. Otherwise, it
      // acts like the "file" loader.
      {
        exclude: [
          /\.html$/,
          // We have to write /\.(js|jsx)(\?.*)?$/ rather than just /\.(js|jsx)$/ because
          // you might change the hot reloading server from the custom one to Webpack's
          // built-in webpack-dev-server/client?/, which would not get properly excluded
          // by /\.(js|jsx)$/ because of the query string. Webpack 2 fixes this, but for
          // now we include this hack.
